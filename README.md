...
> 选区界面截图待补充。可自行截取：运行 → Alt+Q → 移动鼠标到任意窗口触发高亮 → 截屏 → 存为 `docs/screenshots/screenshot-region.png`，然后取消下面一行的注释：
>
> `<!-- ![截图选区](docs/screenshots/screenshot-region.png) -->`
...
.\build.ps1
# 或直接用 MSBuild
MSBuild Ocris.csproj /p:Configuration=Release /p:Platform=x64
```

输出在 `bin\Debug\Ocris.exe`（或 Release）。

### OCR 模型

首次运行需要 PaddleOCR 推理模型（仓库不入库，避免体积过大）：

1. 下载 [PaddleOCR PP-OCRv4 模型](https://github.com/PaddlePaddle/PaddleOCR/blob/main/doc/doc_ch/PP-OCRv4_introduction.md)（det / cls / rec + 字典 `ppocr_keys.txt`）
2. 解压放到 `bin\Debug\inference\`（或 `bin\Release\inference\`），结构：
   ```
   bin/Debug/inference/
...
> 也可从本仓库的 GitHub Release 附件获取预置模型包。
...
```
Ocris/
├── Models/          数据模型（ConfigModel / OCR / AI 响应）
├── Services/        服务层（截图 / OCR / AI / 配置 / 热键 / 主题 / 窗口检测）
├── ViewModels/      MVVM 视图模型
├── Views/           WPF 窗口（主/截图/设置）
├── Styles/          全局样式
├── Utils/           工具（HotkeyParser / RelayCommand）
...
欢迎提 Issue 反馈问题、PR 贡献代码。

- 代码风格遵循现有约定（C# 5 兼容，详见 `docs/DESIGN.md`）
- 提交信息建议：`类型: 描述`（feat / fix / refactor / docs / chore）
