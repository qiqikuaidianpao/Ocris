...
| 模块 | 功能 | 参考 | 现状 |
|------|------|------|------|
| 截图 | 区域截图 + UI 元素智能识别高亮 | PixPin / Snipaste | ⚠️ 需重建（自研） |
...
| 现状 | 改造目标 |
|------|---------|
| `旧截图引擎`（8 DLL + 反射魔改） | **删除**，新建 `GdiScreenshotEngine`（BitBlt + UIA） |
...
| 旧命名空间（早期遗留） | **统一为 `Ocris`** |

## 7. 路线图

- **Phase 0 — 立项准备**：设计文档（本文）、全局改名 `改名`。
- **Phase 1 — 截图引擎重建（MVP）**：自研截图引擎；新建 GDI 截图引擎；UIA 智能识别；修 4 个 Bug（窗口识别 / 多屏 / 快捷键 / 设置）；配置与密钥收敛。
